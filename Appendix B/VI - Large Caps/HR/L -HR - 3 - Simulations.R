...
L = t(chol(corr_matrix_L_HR))            # nAssets x nAssets
  ...
  for (t in 1:Tdays) {
    ...
    if (length(idx) > 0) {
      for (i in idx) {
        ...
        logJump[i] = sum(logY)            # log of product of Y_k
      }
    }
...
SnP_500_HR = (SnP_500$GSPC.Close[604:1340])
mu_daily_Index <- mean(SnP_500_HR)
sigma_daily_Index <- sd(SnP_500_HR)
S0_index= as.numeric(tail(closing_prices)["2025-08-01", ])

Index_Simulation_L_HR = Merton_Model(S0_index, mu_daily_Index, sigma_daily_Index,13)


#### Stocks part
MUs= Portfolio_Large_Info$Stock_MU_HR     
SIGMAs= Portfolio_Large_Info$Stock_Sigma_HR  
SOs= Portfolio_Large_Info$Stock_So     
Tickers= Portfolio_Large_Info$Stock_Ticker

Price_paths_L_HR = list()
nStocks = nAssets-1

for (i in 1:nStocks) {
  indiv_price_paths = Merton_Model(S0 = SOs[i],mu_daily = MUs[i],sigma_daily = SIGMAs[i],asset_index = i)
  Price_paths_L_HR[[Tickers[i]]] = indiv_price_paths
}
...
IndivualWeigth = list()
Portfolio_0_S0_L_EW_HR = matrix(0, nrow = MC, ncol = Tdays+1)

for (i in 1:length(Tickers)) {
  ticker = Tickers[i]
  IndivualWeigth[[ticker]] = Price_paths_L_HR[[ticker]] * Table_LEW_HR[i,3]
  Portfolio_0_S0_L_EW_HR  = Portfolio_0_S0_L_EW_HR  + IndivualWeigth[[ticker]]
}


####################### VOLATILITY WEIGHTED PORTFOLIO #######################
IndivualWeigth <- list()
Portfolio_0_S0_L_VW_HR = matrix(0, nrow = MC, ncol = Tdays+1)

for (i in 1:length(Tickers)) {
  ticker <- Tickers[i]
  IndivualWeigth[[ticker]] = Price_paths_L_HR[[ticker]] * Table_LVW_HR[i,3]
  Portfolio_0_S0_L_VW_HR <- Portfolio_0_S0_L_VW_HR + IndivualWeigth[[ticker]]
}

####################### INVESERLY VOLATILITY WEIGHTED PORTFOLIO ####################### 
IndivualWeigth <- list()
Portfolio_0_S0_L_IW_HR<- matrix(0, nrow = MC, ncol = Tdays+1)

for (i in 1:length(Tickers)) {
  ticker <- Tickers[i]
  IndivualWeigth[[ticker]] <- Price_paths_L_HR[[ticker]] * Table_LIW_HR[i,3]
  Portfolio_0_S0_L_IW_HR <- Portfolio_0_S0_L_IW_HR + IndivualWeigth[[ticker]]
}

################ CORRELATION ################
### PATHWISE CORRELATION
portfolio_logrets <- t(apply(Portfolio_0_S0_L_EW_HR, 1, function(x) diff(log(x))))
index_logrets <- t(apply(Index_Simulation_L_HR, 1, function(x) diff(log(x))))
